class CivicsBuilder {
    build(civicsFile = {}, filterFn = null) {
        ...
        for (const [civicName, civicData] of Object.entries(civicsFile)) {
            ...
            defs[civicName] = { is_nomadic: this._extractNomadic(civicData), yes, no };
        }
     ...
     * Returns 'yes' when this civic requires a nomad empire, 'no' when it requires a
     * non-nomad empire and null when the game places no restriction on it.
     */
    _extractNomadic(civicData) {
        const blocks = [];
        for (const key of ['potential', 'possible']) {
            const block = civicData[key];
            if (!block) continue;

...
class TraitsBuilder {
    ...
    build(parsedData, filterFn) {
        ...
        for (const [id, data] of Object.entries(parsedData)) {
            ...
            // Extract the homeworld planet classes this trait is limited to, if any
            const allowedPlanetClasses = this._extractAllowedPlanetClasses(data);
            ...
            // Add allowed_planet_classes if it exists
            if (allowedPlanetClasses.length > 0) {
                traitDef.allowed_planet_classes = allowedPlanetClasses;
            }

            // Some traits are only offered to empires with a specific origin or civic
            const allowedOrigins = this._extractList(data.allowed_origins);
            if (allowedOrigins.length > 0) {
                traitDef.allowed_origins = allowedOrigins;
            }

            const allowedCivics = this._extractList(data.allowed_civics);
            if (allowedCivics.length > 0) {
                traitDef.allowed_civics = allowedCivics;
            }
    ...
    /**
     * Extract allowed_planet_classes from trait data
     * Traits with this property can only be picked when the empire's homeworld is one of
     * these planet classes (e.g. the Nomads traits require an ark: pc_ark)
     * @param {Object} data - Trait data object
     * @returns {Array} Array of planet class IDs (e.g. ['pc_ark', 'pc_habitat'])
     * @private
     */
    _extractAllowedPlanetClasses(data) {
        const planetClasses = [];

        if (data.allowed_planet_classes) {
            // allowed_planet_classes can be a single string or an array
            if (typeof data.allowed_planet_classes === 'string') {
                planetClasses.push(data.allowed_planet_classes);
            } else if (Array.isArray(data.allowed_planet_classes)) {
                planetClasses.push(...data.allowed_planet_classes);
            }
        }
    ...
    /**
     * Normalise a trait property that can hold a single value or a list

class OriginsBuilder {
    build(originsFile = {}, filterFn = null, traitsLookup = null, civicsFiles = []) {
        ...
        for (const [originName, originData] of Object.entries(originsFile)) {
            ...
            const yes = { authorities: [], civics: [], ethics: [], species: [], culture: [] };
            const no  = { authorities: [], civics: [], ethics: [], species: [], culture: [] };
            ...
            yes.species = [...new Set(yes.species)];
            no.species  = [...new Set(no.species)];
    ...
    _extractReqs(node, yes, no) {
        for (const [key, val] of Object.entries(node)) {
            ...
            for (const v of values) {
                ...
                else if (key === 'species_class') this._handle(v, yes.species, no.species); // Origins use species_class
                else if (key === 'species_archetype') this._handle(v, yes.species, no.species); // Origins also use species_archetype

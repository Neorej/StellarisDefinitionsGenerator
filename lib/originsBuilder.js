class OriginsBuilder {
    build(originsFile = {}, filterFn = null, traitsLookup = null, civicsFiles = []) {
        ...
        for (const [originName, originData] of Object.entries(originsFile)) {
            ...
            const yes = { authorities: [], civics: [], ethics: [], species_class: [], species_archetype: [], culture: [], traits: [] };
            const no  = { authorities: [], civics: [], ethics: [], species_class: [], species_archetype: [], culture: [], traits: [] };
            ...
            yes.traits = [...new Set(yes.traits)];
            no.traits  = [...new Set(no.traits)];

            // Flatten single-element OR groups in yes arrays
            this._flattenSingleElementGroups(yes);

            defs[originName] = { is_nomadic: this._extractNomadic(originData), yes, no };
        }
    ...
    /**
     * Nomad empires (is_nomadic = yes) can only take part of the content.
     * Returns 'yes' when this origin requires a nomad empire, 'no' when it requires a
     * non-nomad empire and null when the game places no restriction on it.
     */
    _extractNomadic(originData) {
        const blocks = [];
        for (const key of ['potential', 'possible']) {
            const block = originData[key];
            if (!block) continue;
            // A definition can hold multiple potential/possible blocks
            blocks.push(...(Array.isArray(block) ? block : [block]));
        }

        for (const block of blocks) {
            if (typeof block !== 'object' || block === null) continue;
            if (!('is_nomadic' in block)) continue;

            // Duplicate keys are collected into an array by the parser
            const value = Array.isArray(block.is_nomadic) ? block.is_nomadic[0] : block.is_nomadic;
    ...
    _extractTraitRequirements(traitsBlock, yes, no, traitsLookup) {
        ...
        for (const traitId of traitIds) {
            ...
            if (traitData.allowed_archetypes) {
                ...
                // Add as an OR group (need one of these archetypes), unless an identical
                // group is already there because the origin adds several such traits
                const alreadyAdded = yes.species_archetype.some(group =>
                    Array.isArray(group) && group.join() === archetypes.join());
            ...
            // A trait the origin force-adds makes its opposites impossible to pick
            if (traitData.opposites) {
                const opposites = Array.isArray(traitData.opposites)
                    ? traitData.opposites
                    : [traitData.opposites];

                no.traits.push(...opposites);
            }
        }
    }
